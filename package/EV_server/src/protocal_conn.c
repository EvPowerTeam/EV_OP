...
DateTime  HB_DateTime[30]; 
int	  HB_DateTime_cnt = 0;
...
DateTime  get_DateTime(void)
{
	...
	datime.year = ttm->tm_year + 1900;
	datime.mon = ttm->tm_mon+1;
	datime.day = ttm->tm_mday;
	datime.hour = ttm->tm_hour;
	datime.min = ttm->tm_min;
	datime.second = ttm->tm_sec;
...
{
	char leap = 0;
	int year = now->year - last->year;
	char day = now->day - last->day;
	char hour = now->hour - last->hour;
	char min = now->min - last->min;
	char second = now->second - last->second;
	short days_lastyear = 0;
	short days_nowyear = 0;
	char last_mon  = last->mon;
	char now_mon = now->mon;
	
//	printf("last_time:%d %d %d %d %d %d\n", last->year, last->mon, last->day, last->hour, last->min, last->second);
//	printf("now_time:%d %d %d %d %d %d\n", now->year, now->mon, now->day, now->hour, now->min, now->second);
	...
	int total = year *(365+leap) *24 * 60 * 60 +  (days_nowyear - days_lastyear)*24*60*60 + day*24*60*60 + hour*60*60 +min*60 + second ; 

//	printf("diff_time函数 total = %d\n", total);
	...
	switch(CMD){
		case  HOUR:   	return (total / 3600);  		break;
		case  MIN:	return (total / 60);			break;
		case  SECOND:	return total;				break;
		case  DAY:	return (total / (3600*24));		break;
		case MONTH:	break;
		case YEAR:	break;
	}
	return -1;
...
//	printf("\n");
	...
	ConnectREQ.JoinConnectVersion[1] = (u8)JOIN_And_CONN_VERSION;
	ConnectREQ.JoinConnectVersion[0] = (u8)((0));
	printf("jon:%d\n", (u8)((JOIN_And_CONN_VERSION)*100));
//	printf("conn  version:%d.%d\n", ConnectREQ.JoinConnectVersion[0], ConnectREQ.JoinConnectVersion[1]);
	ConnectREQ.ChargerFWVersion[0]   = (u8)(CHARGER_VERSION ); 
	ConnectREQ.ChargerFWVersion[1]   = (u8)(CHARGER_VERSION >> 8); 
...
//	ConnectCF->AddCID[3] = (u8)cid;
	...
	CS_FUP->FirmwareVersion[0] = (u8)(0);
	CS_FUP->FirmwareVersion[1] = (u8)(JOIN_And_CONN_VERSION);

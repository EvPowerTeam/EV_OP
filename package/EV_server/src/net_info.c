...
int CServer_send(int sockfd, unsigned char *buff, unsigned int len)
{
	...
	My_AES_CBC_Encrypt(KEYB,input_buff, slen, send_buff+4);
	send_buff[0] = buff[8];
	send_buff[1] = buff[9];
	send_buff[2] = buff[10];
	send_buff[3] = buff[11];
...
//	printf("\n发送命令:%#x\n 长度:%d\n", buff[7], slen+4);
	...
	if( (res = read(sockfd, recv_buff, sizeof(recv_buff))) <= 0){
...
int CServer_recv_deal(unsigned char *recv_buff, unsigned int len)
{
	...
	unsigned int slen = 0;
	int i;
	My_AES_CBC_Decrypt(KEYB, recv_buff, len, buff);
	...
	u16 Crc = getCRC(re_buff, slen-2);
	if( ((u8*)&Crc)[0] != crc[1] || ((u8*)&Crc)[1] != crc[0]){
...
//	if(*(u16*)crc != getCRC(re_buff, slen-2)){
	...
	switch(re_buff[7]){
		...
		case 0x43:
			...
			HB_DateTime[HB_DateTime_cnt] = get_DateTime();
			HB_DateTime_cnt++;
#endif
		break;
	}
...
int CServer_conn_HB(int sockfd)
{
	...
	My_AES_CBC_Encrypt(KEYB, input_buff, len, recv_buff); 
...
// 报错。。。。
	My_AES_CBC_Encrypt(KEYB, input_buff, len, aes_buff); 
	My_AES_CBC_Decrypt(KEYB, aes_buff, len, input_buff);
...
//	alarm(0);
	memset(input_buff, 0, sizeof(input_buff));
	My_AES_CBC_Decrypt(KEYB, recv_buff, res, input_buff);
...
int  join_conn_serv(int sockfd, struct connect_serv *addr)
{
	...
	int len = 0,  slen = 0;
	int i ;
	 unsigned char * p_req = (u8 *)&ConnectREQ;
	signal(SIGALRM, commu_alarm);
	...
	printf("接收到数据len=%d...\n", len);
	// 接收信息进行解密
	My_AES_CBC_Decrypt(KEYA, recv_buff, len, outputMessage);

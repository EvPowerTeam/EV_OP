...
ev_int  charger_serv(const ev_int fd, const ev_int cmd, CHARGER_INFO_TABLE *charger,  BUFF *bf, ev_int *index);
void protocal_init_head(ev_uchar CMD,  ev_char *p_str, ev_uint cid);
...
void * thread_main(void * arg)
{
        ...
        if(pthread_detach(pthread_self()) < 0)
	{
        ...
        if ( (bf.recv_buff = (ev_uchar *)malloc(MAX_LEN)) == NULL)
                goto exitt;
        if ( (bf.send_buff = (ev_uchar *)malloc(MAX_LEN)) == NULL)
                goto exitt;
        if ( (bf.val_buff  = (ev_char * )malloc(MAX_LEN)) == NULL)
                goto exitt;
...
// 充电协议处理程序
int  
charger_serv(const ev_int fd, const ev_int cmd, CHARGER_INFO_TABLE *charger,  BUFF *bf, ev_int *index)
{
        ev_char   *key_addr = NULL, *sptr = NULL, CID_flag = 0, ChargerCnt;
	ev_uint   len, i, n, ops, err, tmp_4_val, declen = 0, enclen = 0, CID = 0;
	ev_ushort CRC = 0x1d0f, SubMode = 0, tmp_2_val = 0;
        time_t tm;
        FILE *file;
	...
	CRC = getCRC(bf->recv_buff, len-2);
        if(CRC !=  *(ev_ushort *)(bf->recv_buff+len-2))
	{	
                debug_msg("CRC校验失败:CID[%d] ...", *(ev_uint *)(bf->recv_buff + 5));
		...
		case	CHARGER_CMD_CHARGE_REQ:	// 0X54 充电请求
			...
			charger->is_charging_flag = 0;
		                ...
		                if ( (file = fopen(bf->val_buff, "ab+")) != NULL)
				{
					...
					bf->send_buff, *(unsigned short*)(bf->recv_buff + 34), *(int *)(bf->recv_buff + 13), 0xEF);
...
replay58:            
       ...
       case    CHARGER_CMD_CHARGE_ERROR:   // 0x61 接收开始充电
               ...
               } else if (bf->recv_buff[9] == 4)
               {
                ...
                gernal_command(fd, CHARGER_CMD_HB_R, charger, bf); 
                bf->ErrorCode = ESTART_CHARGE_ERR;
#endif
                return 0; 
        break;
...
//                        break;
        ...
        } else if (task->way == SERVER_WAY)
        {
            ...
            } else if (WAIT_CMD_YUYUE == task->cmd)
            {
		        ...
		        sprintf(val, "/PushMessage/pushMessage?");
...
//                    memcpy(val_buff, task->u.yuyue.uid, 32);
            ...
            {
                   if ( (val = (char *)calloc(300, 1)) != NULL)
                   {
                        memset(val_buff, 0, sizeof(val_buff));
                        memcpy(val_buff, task->u.stop_charge.uid, 32);
		        sprintf(val, "/PushMessage/pushMessage?");
...
void   check_charger_network(void)
{
        ...
        for (i = 0; i < charger_manager.present_charger_cnt; i++)
        {
                        if (ChargerInfo[i].free_cnt_flag == 1 && ChargerInfo[i].free_cnt > 15)
                        {
...
void * pthread_service_receive(void *arg)
{
    ...
    char    i;
    char    have_handle_flag, update_cnt, val_buff[250] = {0}, name[10];
...
//       debug_msg("!CID = %d, !CID_TMP= %d, !CMD=%d, !CMD_TMP=%d\n", CID, CID_TMP, CMD, CMD_TMP); 
      ...
      switch (CMD)
      {
        ...
        case    WAIT_CMD_ALL_CHAOBIAO:
                cmd.u.chaobiao.start_time = init_time;
                cmd.u.chaobiao.end_time = time(0);//end_time;
                check_have_all_handle(WEB_WAY, &cmd);
        break;
...
void *load_balance_pthread(void *arg)
{
      ...
      // load balance
      // 读取充电中断网的电桩，程序重启的情况,针对PresentMode =
      // 46的处理
       for (i = 0; i < charger_manager.present_charger_cnt; i++) {
                if(ev_uci_data_get_val(send_buff, 10, "chargerinfo.%s.PresentMode", ChargerInfo[i].tab_name) >= 0) {
                        if (!strcmp(send_buff, "46") && ev_uci_data_get_val(send_buff, 10, "chargerinfo.%s.off_net_charging", ChargerInfo[i].tab_name) >= 0) {
                                if (strcmp(send_buff, "1") == 0){
                                        debug_msg("有电桩充电中断网:%d", ChargerInfo[i].CID);
                                        no_change_index[i] = 1;
                                        ChargerInfo[i].real_time_current = 16;
                                        ChargerInfo[i].real_current = 7;
                                        ChargerInfo[i].is_charging_flag = 1;
...
//                        printf("max_current = %d\n", charger_manager.limit_max_current);
                ...
                for (i = 0; i < CNT; i++)
                {
                        ...
                        if (ChargerInfo[i].free_cnt_flag == 1 && ChargerInfo[i].free_cnt > 10)
                        {
                                ...
                                if (ChargerInfo[i].is_charging_flag == 1)
                                {
                                  ev_uci_save_action(UCI_SAVE_OPT, true, "1", "chargerinfo.%s.off_net_charging", ChargerInfo[i].tab_name);
			...
			if(ChargerInfo[i].free_cnt_flag == 1)
			 {
				    ...
				    off_net_cnt++;	
                                   if (ChargerInfo[i].is_charging_flag == 1 && off_net_charging[i] == 1)
                                   {
                                       no_change_index[i] = 1;
                                       ChargerInfo[i].real_time_current = 16;
                                       ev_uci_save_action(UCI_SAVE_OPT, true, "0", "chargerinfo.%s.off_net_charging", ChargerInfo[i].tab_name);
                                   } 
        ...
        for ( i = 0; i < CNT; i++)
        {
            if (balance[i].charging_flag == 1)
            {
                if (no_change_index[i] == 1)
                {
                    ...
                    printf("最终分配的电流no[%d]-->%d\n", i, ChargerInfo[i].real_time_current);
                }
                else
                {
...
void  charger_info_init(int select)
{
	...
	char name[20][10] = {{0}}, info[200] = {0}, i = 0, j=0, charg_cnt = 0;
        int data, ii; 
        unsigned char tmp[2];

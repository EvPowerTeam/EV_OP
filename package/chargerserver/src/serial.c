...
int   power_bar_ctrl_send(int serial_fd, unsigned char pwn_status, unsigned char color, unsigned char color_cnt)
{
    ...
    send_buff[0] =  0x53;
    send_buff[1] = pwn_status;
    send_buff[2] = color;
    send_buff[3] = color_cnt + 0x30;
    send_buff[4] = 0x45;
    return UART_Send(serial_fd, send_buff, 5);
}
...
int power_bar_ctrl_recv(int serial_fd)
{
    ...
    if ( UART_Recv(serial_fd, recv_buff, 5) != 5)
        return FALSE;
    if (recv_buff[1] != 0x01 || recv_buff[2] != 0x01 || recv_buff[3] != 0x01)
        return FALSE;
    return TRUE;
}
